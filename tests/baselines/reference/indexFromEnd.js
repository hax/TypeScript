...
// Valid usage - simple identifier
var index = 1;
var last = arr[arr.length - index]; // Should be arr[arr.length - index]
// Valid usage - literal
var secondLast = arr[arr.length - 2]; // Should be arr[arr.length - 2]
// Valid usage - expression with parentheses
var n = 1;
var m = 2;
var fromEnd = arr[arr.length - (n + m)]; // Should be arr[arr.length - (n + m)]
// Valid usage - method call
function getIndex() {
    return 1;
}
var withMethod = arr[arr.length - getIndex()];
// Valid usage - property access
var obj = { idx: 1 };
var withProperty = arr[arr.length - obj.idx];
// Type checking - should error for non-number
var str = "1";
var invalid = arr[arr.length - str]; // Error: Index from end operator requires an operand of type 'number'
// Should work with string arrays
var strArr = ["a", "b", "c"];
var lastStr = strArr[strArr.length - 1];
// Should work with tuples
var tuple = [1, "test", true];
var lastInTuple = tuple[tuple.length - 1];
// Should work with readonly arrays
var readonlyArr = [1, 2, 3];
var lastReadonly = readonlyArr[readonlyArr.length - 1];
